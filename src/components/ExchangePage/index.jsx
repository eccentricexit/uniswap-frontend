...
function getMarketRate(
  ...
  } else if (swapType === SWAP_TYPE.TOKEN_TO_ETH) {
    return getExchangeRate(inputReserveToken, inputDecimals, inputReserveETH, 18, invert)
  } else if (swapType === SWAP_TYPE.TOKEN_TO_TOKEN) {
    const factor = ethers.utils.bigNumberify(10).pow(ethers.utils.bigNumberify(18))
...
export default function ExchangePage({ initialCurrency, sending }) {
  ...
  const contract = swapType === SWAP_TYPE.ETH_TO_TOKEN ? outputExchangeContract : inputExchangeContract
  ...
  const { minimum: dependentValueMinumum, maximum: dependentValueMaximum } = calculateSlippageBounds(
    dependentValue,
    swapType === SWAP_TYPE.TOKEN_TO_TOKEN,
  ...
  return (
    <>
      ...
      <TransactionDetails
        ...
        sending={sending}
        tokenReservesInput={tokenReservesInput}
        tokenReservesOutput={tokenReservesOutput}
        reserveETHInput={reserveETHInput}
        reserveETHOutput={reserveETHOutput}
        swapType={swapType}
